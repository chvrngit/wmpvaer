...
resultsd1<-replace(resultsd1,resultsd1=="NaN","No Answer")
...
real_eigs<-Re(object@eigs);
eigs<-as.matrix(real_eigs);
w<-computeWilks(real_eigs);
leigs<-nrow(as.matrix(real_eigs));
weigs<-ncol(as.matrix(real_eigs));
...
#' @param omega Is numeric vector,eigenvalues of a 1-way MANOVA (E-Inv)*H matrix, length=>2 & =<10 
#' @examples
#'     suppressPackageStartupMessages(library(Rmpfr)); 
#'     library(wmpvaer);
...
if( (any(is.na(omega)))==TRUE ){stop("omega must be numeric")};
...
if( (l_omega>10)==TRUE ){stop("program restricted to models between 2 and 10 dependent vars") };
