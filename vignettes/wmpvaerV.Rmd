...
Recall that the p-value (type 2 error) is defined to be $Pr(x<=log(Wilks)\vert\Omega_{observed})$. Conversely the power value is $Pr(x>log(Wilks)\vert\Omega_{observed})$ or 1-(type 2 error). So $Pr(log(Wilks)<=x\vert \Omega=0{p,p})$ (assuming  $\Omega=0{p,p}$) is the probability of a type 1 error (or probability of a false positive, incorrectly rejecting a null hypothesis when it should not be rejected). The Butler/Wood equations used to estimate this function are the same as those used to estimate a p-value when $\Omega$ is an observed value instead of an assumed null value $\Omega=0{p,p}$. In a study design $G_{0}(x):= Pr(log(Wilks)<=x\vert \Omega=0{p,p})$ is often referred to as alpha ($\alpha$).  
